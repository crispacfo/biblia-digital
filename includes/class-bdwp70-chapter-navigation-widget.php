...
class BDWP70_Chapter_Navigation_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			__( 'Bíblia Digital - Chapters of the current book', 'estudobiblico-biblia-digital' ),
			array(
				'classname'                   => 'bdwp70_chapter_navigation_widget',
				'description'                 => __( 'Displays a card with the book title, the current chapter, a chapter grid, and the active translation on Bible pages.', 'estudobiblico-biblia-digital' ),
	...
	public function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Optional title:', 'estudobiblico-biblia-digital' ); ?></label>
			<input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>">
		</p>
		<p class="description"><?php esc_html_e( 'The card appears automatically only on Bíblia Digital book or chapter pages.', 'estudobiblico-biblia-digital' ); ?></p>
		<?php
	}
